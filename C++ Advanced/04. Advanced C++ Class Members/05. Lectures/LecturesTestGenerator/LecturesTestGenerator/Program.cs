...
namespace LecturesTestGenerator
{
    class Resource
    {
    ...
    class Program
    {
        ...
        {
            GenerateTest(testName: "test.001", duplicatesCount: 1, presentationsCount: 2, demosCount: 3, videosCount: 1);
            GenerateTest(testName: "test.002", duplicatesCount: 0, presentationsCount: 1, demosCount: 0, videosCount: 0);
            GenerateTest(testName: "test.003", duplicatesCount: 0, presentationsCount: 0, demosCount: 1, videosCount: 0);
            GenerateTest(testName: "test.004", duplicatesCount: 0, presentationsCount: 0, demosCount: 0, videosCount: 1);
            GenerateTest(testName: "test.005", duplicatesCount: 3, presentationsCount: 10, demosCount: 30, videosCount: 10);

            GenerateTest(testName: "test.006", duplicatesCount: 0, presentationsCount: 20, demosCount: 1, videosCount: 1);
            GenerateTest(testName: "test.007", duplicatesCount: 10, presentationsCount: 1, demosCount: 30, videosCount: 1);
            GenerateTest(testName: "test.008", duplicatesCount: 20, presentationsCount: 1, demosCount: 1, videosCount: 10);
            GenerateTest(testName: "test.009", duplicatesCount: 30, presentationsCount: 20, demosCount: 3, videosCount: 27);
            GenerateTest(testName: "test.010", duplicatesCount: 40, presentationsCount: 12, demosCount: 13, videosCount: 25);
        }

        static void GenerateTest(string testName, int duplicatesCount, int presentationsCount, int demosCount, int videosCount)
        {
            ...
            List<Resource> duplicates = new List<Resource>();
            while (duplicates.Count < duplicatesCount)
            {
                Resource original = random.NextFrom(orderedResources);
                Resource copy = new Resource(original.id, original.type, random.NextUrl());
                duplicates.Add(copy);
            }

            foreach (var duplicate in duplicates)
            {
                int firstMatchingIndex = inputResources.FindIndex(r => r.id == duplicate.id);
                inputResources.Insert(random.Next(0, firstMatchingIndex + 1), duplicate);
            }
