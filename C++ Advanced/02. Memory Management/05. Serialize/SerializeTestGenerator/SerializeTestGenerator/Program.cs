...
namespace SerializeTestGenerator
{
    class Program
    {
        ...
        private static void GenerateTest(string testName, int companiesCount, int minEmployees, int maxEmployees)
        {
            ...
            System.IO.File.WriteAllText(testName + ".in.txt", string.Join(System.Environment.NewLine, companies.Select(c => c.ToString()))
                + System.Environment.NewLine + "end" + System.Environment.NewLine);

            System.IO.File.WriteAllText(testName + ".out.txt", companies.Count + " " + string.Join(" ", companies.Select(c => string.Join(" ", c.ToByteArray().ToList()))));
        }
    }
}
